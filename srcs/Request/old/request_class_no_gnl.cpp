...
int Request::parse()
{
    if (this->body_size || this->chunked_encoding) // if read was interrupted while reading body, we go directly to body_parsing
    ...
    {
        if ((ret = read(this->fd, buf, BUF_SIZE)) <= 0)
        {
            // ret == -1 => error reading: could be that the client socket has no data to send (EAGAIN/EWBLOCK) or that it has been shut down by a signal
            // ret == 0 ==> no more to read from connection (->close connection)
            free (buf);
            return ret; 
        }
        buf[ret] = 0;
...
void Request::cut_buf_line(std::string &line)
{
    // std::cout << "buf: [" << this->buffer[this->fd] << "]" << std::endl;
    size_t pos;
...
void Request::parse_headers()
{
    ...
    // store req. line
    this->cut_buf_line(line);
    if (line.empty()) // skip one empty line
        this->cut_buf_line(line);
...
int Request::read_normal() // we read the number of bytes specified by "content-lenght"
{
    this->body = this->buffer.substr(0, this->body_size);
    this->body_size = this->body_size - this->body.size(); // remaining bytes to read
    if (this->body_size == 0)
        return SUCCESS;
    int ret(0);
    int received(0);
